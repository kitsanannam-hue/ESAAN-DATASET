...
class ThaiJazzDatasetBuilder:
    ...
    def analyze_content_for_features(self, text: str, page_num: int) -> List[Dict]:
        ...
        feature_patterns = {
            ...
            'rhythm': r'rhythm(?:ic)?\s*(?:pattern|feature|analysis)',
            'melody': r'melod(?:y|ic)\s*(?:contour|feature|extraction)',
            'harmony': r'harmon(?:y|ic)\s*(?:analysis|feature|progression)',
            'timbre': r'timbr(?:e|al)\s*(?:feature|analysis|characteristic)',
            ...
            'thai_mode': r'thang|ทาง|thai\s+(?:mode|scale)',
            'jazz_chord': r'(?:jazz\s+)?chord\s*(?:progression|type|voicing)',
            'improvisation': r'improvis(?:ation|e|ing)',
            'ornamentation': r'ornament(?:ation|s)?|กรอ|วน',
        }
